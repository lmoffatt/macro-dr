...
struct to_experiment {
  ...
  static auto get_arguments() {
    return std::make_tuple(
        grammar::argument(C<const io::myDataFrame<double> &>{}, "data_frame"),
...
struct to_DataFrame<
  ...
  static auto get_arguments() {
    return std::make_tuple(grammar::argument(
        C<const evidence::Likelihood_Analisis<Parameters_Distribution,
                                              Parameters_Values, ExperimentData,
                                              logLikelihood> &>{},
        "analysis"));
  }
  ...
  static auto get_arguments() {
    return std::make_tuple(
        grammar::argument(C<typename std::mt19937_64::result_type>{},
                          "initseed", typename std::mt19937_64::result_type(0)),
...
struct likelihoodtest {
  ...
  static auto run(std::size_t initseed, const Experiment &e, const Model &m,
                  ...
                  const std::string algorithm, double BiNumber, double VaNumber,
                  double min_P, double tolerance, double eps_Gradient,
                  bool eps_adjust, bool center_Gradient,
...
struct likelihoodtest_derivative {
  ...
  static auto run(std::size_t initseed, const Experiment &e, const Model &m,
                  ...
                  const ParametersDistribution &prior,
                  const std::string algorithm, double BiNumber, double VaNumber,
                  double min_P, double tolerance, std::size_t n_sub_intervals,
                  double max_dt, std::size_t nsamples, double pvalue) {
    std::cerr << "\nparameters\n" << p;
    std::cerr << "\n initseed=" << initseed << "\n";
    std::cerr << "\n n_sub_intervals=" << n_sub_intervals << "\n";
    std::cerr << "\n nsamples=" << nsamples << "\n";
...
struct Evidence {
  ...
  run(const Experiment &e, const Model &m, const ParametersDistribution &p,
      ...
      std::vector<double> landa, std::vector<std::vector<double>> landa_50_hill,
...
struct Evidence_emcee {
  ...
  static auto get_arguments() {
    return std::make_tuple(
        ...
        grammar::argument(C < std::map<std::size_t, std::size_t>>{},
                          "ana_sampling_cycles"));
    // evidence_works = evidence_emcee ( singleLigandExperiment = mySimulation
