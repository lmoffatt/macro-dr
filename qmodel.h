...
class Markov_Transition_rate
{
    ...
    void init(Matrix_Decompositions::eigensystem_type&& eig)
    {
        ...
        //assert(Frobenius_test::test(Qrun(),V()*landa()*W(),1, std::sqrt(std::numeric_limits<double>::epsilon())));
    }
...
public:
    ...
    static Op_void test(const Matrix_Decompositions::eigensystem_type& e,double tolerance)
    {
        return landa::test(std::get<1>(e),tolerance);

    }

    static myOptional_t<Markov_Transition_rate> evaluate(M_Matrix<double>&& _Qrun,
                                                         const M_Matrix<double>& _g,
                                                         double tolerance)
    {
        ...
        if (eig)
        {
            if (auto eigtest=test(eig.value(),tolerance); eigtest.has_value())
                return Op(Markov_Transition_rate(std::move(_Qrun),_g,std::move(eig).value()));
            else
                return Op(false,"invalid eigenvalues "+eigtest.error());
        }
        else return Op(false,"eigenvalue decomposition fails:"+eig.error());
    }
...
template<>
class Derivative<Markov_Transition_rate>
{
...
public:
    ...
    static Op_void test(const Derivative_t<Matrix_Decompositions::eigensystem_type>& e,double tolerance)
    {
    ...
    auto  calc_Peq()const
    {
        ...
        M_Matrix<double> laexp(landa().f().size(),landa().f().size(),Matrix_TYPE::DIAGONAL);
...
struct Markov_Transition_step_single_minimum
{
    ...
    std::size_t n;
    Markov_Transition_step_single_minimum(const Markov_Transition_step_single& x_);
    Markov_Transition_step_single_minimum& operator*=(const Markov_Transition_step_single& x_);
};
...
public:
    ...
    Markov_Transition_step(const Markov_Transition_rate& Qx, std::size_t nsamples, double fs,double min_p)
        :P_{},/*ladt_{},exp_ladt_{},*/g_{Qx.g()},nsamples_{nsamples},dt_{1.0/fs*nsamples}, min_p_{min_p}{
        init(Qx);}

    Markov_Transition_step(std::size_t n_sub_samples,const Markov_Transition_rate& Qx, std::size_t nsamples, double fs,double min_p)
        :P_{},/*ladt_{},exp_ladt_{},*/g_{Qx.g()},nsamples_{nsamples},dt_{1.0/(fs*n_sub_samples)*nsamples}, min_p_{min_p}{
        init(Qx);
    }


    Markov_Transition_step(M_Matrix<double>&&  P,
                           M_Matrix<double>&& g, std::size_t nsamples, double fs,double min_p)
        :
          P_{std::move(P)},
          //          P_{Probability_transition::normalize(std::move(P),min_p)},
          g_{std::move(g)}, nsamples_{nsamples}, dt_{(1.0*nsamples)/fs}, min_p_{min_p}{}
...
template<>
class Derivative<Markov_Transition_step>
{
protected:
...
public:
    ...
    Derivative(const Derivative<Markov_Transition_rate>& Qx, std::size_t nsamples, double fs,double min_p)
        :P_{},/*ladt_{},exp_ladt_{},*/g_{Qx.g()},nsamples_{nsamples},dt_{1.0/fs*nsamples}, min_p_{min_p}{
        init(Qx);}

    Derivative(std::size_t n_sub_samples,const Derivative<Markov_Transition_rate>& Qx, std::size_t nsamples, double fs,double min_p)
        :P_{},/*ladt_{},exp_ladt_{},*/g_{Qx.g()},nsamples_{nsamples},dt_{1.0/(fs*n_sub_samples)*nsamples}, min_p_{min_p}{
        init(Qx);
    }


    Derivative(Derivative<M_Matrix<double>>&&  P,
                           Derivative<M_Matrix<double>>&& g, std::size_t nsamples, double fs,double min_p)
        :
          P_{std::move(P)},
          //          P_{Probability_transition::normalize(std::move(P),min_p)},
          g_{std::move(g)}, nsamples_{nsamples}, dt_{(1.0*nsamples)/fs}, min_p_{min_p}{}
    ...
    Derivative<Markov_Transition_step>& operator*=(const Derivative<Markov_Transition_step>& other)
    {
        P_=P_*other.P();
...
struct Markov_Transition_step_double_minimum
{
    ...
    double min_p;
    double tolerance;
    Markov_Transition_step_double_minimum(const Markov_Transition_step_double&x_);
    Markov_Transition_step_double_minimum operator*=(const Markov_Transition_step_double& x_);
};

template<>
struct Derivative<Markov_Transition_step_double_minimum>
{
    std::size_t n;
...
public:
    Markov_Transition_step_double(M_Matrix<double>&&  P,
        ...
        :Markov_Transition_step(std::move(P),std::move(g),nsamples,fs,min_p),
          gmean_i_{gmean_i}, gtotal_ij_{gtotal_ij},gmean_ij_{gmean_ij}, gtotal_sqr_ij_{gtotal_sqr_ij},gsqr_i_{gsqr_i},gvar_i_{gvar_i},gtotal_var_ij_{gtotal_var_ij},gvar_ij_{gvar_ij}{}
    ...
    Markov_Transition_step_double(const Markov_Transition_rate& Qx,std::size_t nsamples, double fs,double min_p)
        :Markov_Transition_step(Qx,nsamples,fs,min_p)
    {   init(Qx);
    }

    Markov_Transition_step_double(std::size_t recursion_number,const Markov_Transition_rate& Qx,std::size_t nsamples, double fs,
                                  double min_p)
        :Markov_Transition_step(Qx,nsamples,fs,min_p)
    {
    ...
    Markov_Transition_step_double(std::mt19937_64& mt,const Markov_Transition_rate& Qx,std::size_t nsamples, double fs,
                                  double min_p, std::size_t numsteps, std::size_t numreplicates)
        :Markov_Transition_step(Qx,nsamples,fs,min_p)
    {
        measure(Qx,mt,numsteps, numreplicates);
    }

    Markov_Transition_step_double(Markov_Transition_step&& step):Markov_Transition_step (step){
        init_by_step_init();

    }


    Markov_Transition_step_double(Markov_Transition_step_double_minimum&& x,  M_Matrix<double> g,double fs, double min_p)
    ...
    constexpr static auto  className=my_static_string("Markov_Transition_step_double");
    static auto get_constructor_fields()
    {
        //M_Matrix<double> const & (self_type::*myP) ()const& =&self_type::P;
        ...
        return   std::make_tuple(
                    ...
                    grammar::field(C<self_type>{},"gtotal_sqr_ij",&self_type::gtotal_sqr_ij),
                    grammar::field(C<self_type>{},"gsqr_i",&self_type::gsqr_i),
                    grammar::field(C<self_type>{},"gvar_i",&self_type::gvar_i),
                    grammar::field(C<self_type>{},"gtotal_var_ij",&self_type::gtotal_var_ij),
    ...
    static double Ee(double x, double y, double exp_x, double exp_y, double eps= std::numeric_limits<double>::epsilon())
    {
        if (sqr(x-y)<eps)
            return exp_x;
        else
    ...
    static double E3(double x, double y, double z, double exp_x, double exp_y, double exp_z, double eps=std::numeric_limits<double>::epsilon())
    {
        if (sqr(x-y)<eps)   // x==y
        {
            if (sqr(y-z)<eps)   // y==z
...
private:
    ...
    void init(const Markov_Transition_rate& Qx)
    {
        ...
        for (std::size_t n1=0; n1<N; n1++)
            for (std::size_t n3=0; n3<N; n3++)
                ...
                {
                    WgV_E3(n1,n3)+=Qx.WgV()(n1,n2)*Qx.WgV()(n2,n3)*E3(ladt[n1],ladt[n2],ladt[n3],exp_ladt[n1],exp_ladt[n2],exp_ladt[n3], min_P());  // optimizable
                }
    ...
    void measure(const Markov_Transition_rate& Qx, std::mt19937_64& mt,std::size_t numsteps, std::size_t nrep)
    ...
    void init_by_step_init()
    {
        ...
        gtotal_var_ij_=(G*P()-P()*G).apply([](double x){return std::abs(x);})*0.5;
    ...
    void init_by_step(const Markov_Transition_rate& Qx, std::size_t recursion_number,double fs)
    {
        ...
        for (std::size_t i=0; i<recursion_number; ++i)
        {
            ...
            step*=step_ini;
            step_ini=Markov_Transition_step_double(std::move(step),step_ini.g(),fs,min_P());
        }
        *this=step_ini;
...
template<>
class Derivative<Markov_Transition_step_double>: public Derivative<Markov_Transition_step>
{
    ...
    Derivative<M_Matrix<double>> gtotal_var_ij_; //variance of the conductance matrix
...
public:
    ...
    Derivative<Markov_Transition_step_double_minimum> getMinimum()const { return Derivative<Markov_Transition_step_double_minimum>(*this);}
   ...
   auto const&  gmean_ij() const {return gmean_ij_;} // conductance matrix
    ...
    /// squared mean conductance for each starting state i and ending state j
   auto const&  gtotal_sqr_ij() const {return gtotal_sqr_ij_;} // conductance matrix

    /// squared mean conductance for each starting state i
   auto const&  gsqr_i() const {return gsqr_i_ ;} // conductance matrix

    /// variance of the mean conductance for each starting state i
   auto const&  gvar_i() const {return gvar_i_;} //variance of the conductance matrix
    /// variance of the mean conductance for each starting state i contributed by the ones ending at state j
   auto const&  gtotal_var_ij() const {return gtotal_var_ij_;} //variance of the conductance matrix

    /// variance of the mean conductance for each starting state i and ending state j
   auto const&  gvar_ij() const {return gvar_ij_;} //variance of the conductance matrix
    ...
    Derivative(std::size_t recursion_number,const Derivative<Markov_Transition_rate>& Qx,std::size_t nsamples, double fs,
                                  double min_p)
        :base_type(Qx,nsamples,fs,min_p)
    {
        init_by_step(Qx,recursion_number,fs);
    ...
    constexpr static auto  className=my_static_string("Markov_Transition_step_double_derivative");
    static auto get_constructor_fields()
    {
        //M_Matrix<double> const & (self_type::*myP) ()const& =&self_type::P;
...
private:
    static double E1(double x){
      return Markov_Transition_step_double::E1(x);
    }
    static double dE1(double x){
        if (std::abs(x)<std::numeric_limits<double>::epsilon()*100) return 0.5;
        else
    ...
    static double dEedx(double x, double y, double exp_x, double exp_y, double eps= std::numeric_limits<double>::epsilon())
    {
        if (sqr(x-y)<eps)
            return exp_x;
        else
    ...
    static Derivative<double> Ee(Derivative<double> x, Derivative<double> y, double exp_x, double exp_y, double eps= std::numeric_limits<double>::epsilon())
    {
        return Derivative<double>(Ee(x.f(),y.f(),exp_x,exp_y,eps),x.x(),
    ...
    {
        return exp_x*((x-y)*(x-z)-(2*x-z-y))/sqr((x-y)*(x-z));
    }
    ...
    static double dE111_dx(double x, double y, double z, double exp_x, double exp_y, double exp_z)
    {
        return dEX_111_dx(x,y,z,exp_x)+dEX_111_dy(y,x,z,exp_y)+dEX_111_dy(z,x,y,exp_z);
    }
    ...
    static double dE3dx(double x, double y, double z, double exp_x, double exp_y, double exp_z, double eps=std::numeric_limits<double>::epsilon())
    {
        if (sqr(x-y)<eps)   // x==y
        {
            if (sqr(y-z)<eps)   // y==z
        ...
        else return dE111_dx(x,y,z,exp_x,exp_y,exp_z); // x!=y!=z!=x
    }

      static Derivative<double> E3(Derivative<double> x, Derivative<double> y, Derivative<double> z, double exp_x, double exp_y, double exp_z, double eps=std::numeric_limits<double>::epsilon())
      {
          ...
          return Derivative<double>(E3(x.f(),y.f(),z.f(),exp_x,exp_y,exp_z,eps),x.x(),std::move(df));
    ...
    void init(const Derivative<Markov_Transition_rate>& Qx)
    {
        ...
        M_Matrix<Derivative<double>> WgV_E3(N,N,Derivative<double>(Qx.Qrun().x()));
        for (std::size_t n1=0; n1<N; n1++)
            for (std::size_t n3=0; n3<N; n3++)
                for (std::size_t n2=0; n2<N; n2++)
                {
                    WgV_E3(n1,n3)+=WgV(n1,n2)*WgV(n2,n3)*E3(ladt[n1],ladt[n2],ladt[n3],exp_ladt[n1],exp_ladt[n2],exp_ladt[n3], min_P());  // optimizable
                }

        gtotal_sqr_ij_=Qx.V()*Derivative<M_Matrix<double>>(WgV_E3)*Qx.W()*2.0;
        for (std::size_t i=0; i<N; ++i)
            for (std::size_t j=0; j<N; ++j)
                if (P().f()(i,j)==0) { gtotal_ij_.f()(i,j)=0;gtotal_sqr_ij_.f()(i,j)=0;}
        ...
        gvar_ij_p=gvar_ij_p.apply([](Derivative<double>const & x){return abs(x);});
    ...
    void init_by_step_init()
    {
        ...
        gtotal_var_ij_.f()=gtotal_var_ij_.f().apply([](double x){return std::abs(x);});
        gmean_ij_=elemDivSafe(gtotal_ij_,P_);
        gtotal_sqr_ij_=gtotal_var_ij_+elemMult(gtotal_ij_,gmean_ij_);
        gvar_ij_=elemDivSafe(gtotal_var_ij_,P_);
        M_Matrix<double> u(N,1,1.0);
    ...
    void init_by_step(const Derivative<Markov_Transition_rate>& Qx, std::size_t recursion_number,double fs)
    {
        std::size_t times=std::pow(2,recursion_number);
        std::size_t n=nsamples();
...
class are_Equal<output,Markov_Transition_step_double>
{
public:
    template<class ostream>
    bool test(const Markov_Transition_step_double& one, const Markov_Transition_step_double& other, ostream& os=std::cerr)const
    {
        ...
        if constexpr (output)
                if (!gtotal_equal) os<<"\n differ in  gtotal_ij !!\n";
        bool gsqr_equal=mytest.test_prod(one.gtotal_sqr_ij(),other.gtotal_sqr_ij(),os);
        if constexpr (output)
...
{
    template<bool output>
    static  bool test_variances(const Markov_Transition_step_double& x, double eps=std::sqrt(std::numeric_limits<double>::epsilon()))
    {
        ...
        else if (!areEqual.test_sum(elemMult(x.gvar_ij(),x.P()),x.gtotal_var_ij()))
        {
            if constexpr(output)
                    std::cerr<<"\n gvar_ij is not equalt to  gtotal_var_ij*u !!!! \n";
            return false;
        }
        else if (!areEqual.test_sum(x.gtotal_var_ij()+elemMult(x.gmean_ij(),x.gtotal_ij()),x.gtotal_sqr_ij()))
        {
            if constexpr(output)
                    std::cerr<<"\n gtotal_var_ij+   elemMult(x.gmean_ij(),x.gtotal_ij()) is not equalt to x.gtotal_sqr_ij() !!!! \n";
            return false;
        }
        else
            return true;
    ...
    template<bool output>
    static  bool test_means(const Markov_Transition_step_double& x, double eps=std::sqrt(std::numeric_limits<double>::epsilon()))
    {
        double gmin=min(x.g());
        double gmax=max(x.g());
        ...
        if (!are_not_more<output,M_Matrix<double>>(true, x.P()*gmax,eps).test(x.gtotal_ij()))
        {
            if constexpr (output)
                    std::cerr<<"\n gtotal_ij is more than gmax="<<gmax<<"!!! \n";
            out=false;
        }
        if (!are_not_less<output,M_Matrix<double>>(true,x.P()*gmin,eps).test(x.gtotal_ij()))
        {
            if constexpr (output)
        ...
        if (!equal.test_prod(elemMult(x.gmean_ij(),x.P()),x.gtotal_ij()))
        {
            if constexpr (output)
                    ...
                    std::cerr<<"\n gtotal_ij is not equal to  elemMult(x.gmean_ij(),x.P())!! \n";
            out= false;
        }
        return out;
    ...
    template <bool output>
    static  bool test(const Markov_Transition_step_double& x, double eps=std::sqrt(std::numeric_limits<double>::epsilon()))
    {
        bool out=true;
...
Markov_Transition_step_double_minimum::Markov_Transition_step_double_minimum(const Markov_Transition_step_double &x):
    n{x.nsamples()}, PPn{x.P()}, PG_n{x.gtotal_ij()*x.nsamples()}, PGG_n{x.gtotal_sqr_ij()*(x.nsamples()*x.nsamples()*0.5)}, min_p(x.min_P()){}
...
public:
    ...
    auto Qx(double x, double tolerance) const { return Markov_Transition_rate::evaluate(Q(x),g(x),tolerance);}

    auto P(const Markov_Transition_rate& aQx, std::size_t n, double fs) const {return Markov_Transition_step(aQx,n,fs,min_P_); }

    auto Pdt(const Markov_Transition_rate& x, std::size_t n, double fs) const {return Markov_Transition_step_double(x,n,fs,min_P_); }
...
public:
    ...
    auto P(const Derivative_t<Markov_Transition_rate>& aQx, std::size_t n, double fs) const {return Derivative<Markov_Transition_step>(aQx,n,fs,min_P_); }
...
public:
    ...
    myOptional_t<Markov_Transition_step> calc_sub_P(X x,std::size_t nsamples,std::size_t n_sub_samples)
    {
        typedef myOptional_t<Markov_Transition_step> Op;
        auto Qx=calc_Qx(x);
        if (!Qx) return Op(false,"fails to calculate Qx: "+Qx.error());
        else
            return Op(Markov_Transition_step(n_sub_samples,Qx.value(),nsamples,fs(),Model().min_P()));
    }

    myOptional_t<Markov_Transition_step> calc_P(X x,std::size_t nsamples,std::size_t num_recursion=0)
    {
        typedef myOptional_t<Markov_Transition_step> Op;
        auto Qx=calc_Qx(x);
        if (!Qx) return Op(false,"fails to calculate Qx: "+Qx.error());
        ...
        {
            if constexpr (std::is_same_v<Markov_Transition_step,Markov_Transition_step_double >)
            {
                if (num_recursion==0)
                {

                    Markov_Transition_step out(Qx.value(),nsamples,fs(),Model().min_P());
                    if (0){
                        std::random_device rd;
                        auto initseed=rd();
                        std::mt19937_64 mt(initseed);
                        Markov_Transition_step out2(mt,Qx.value(),nsamples,fs(),Model().min_P(),  2000, 40000);
                        are_Equal<true,Markov_Transition_step_double>().test(out,out2,std::cerr);
                    }
                    return out;
    ...
    auto  get_P(const step& s,std::size_t num_recursion=0)
    {
        ...
        if ((e-b)>1)
        {
            ...
            if (!PP.has_value()) return Op(false, "fails at start :"+PP.error());
            ...
            for (auto it=b; it!=e; ++it)
            {
                ...
                if (!pc)
                    return Op(false,"fails at interval :"+ToString(*it)+" : "+pc.error());
