...
public:
    ...
    Allosteric_Model(const std::vector<std::string>& conformational_changes,
        ...
        paramNames_=getParameterNamesFrom(conformational_changes_names,conductance_names,conformational_inter);
        std::cout<<paramNames_;
        std::map<std::vector<std::string>,std::size_t> state_to_conformer;
        auto p=periodicity(conformational_changes);

        auto conformational_interactions=fill_conformational_interactions(conformational_inter,conformational_changes.size(),p);
        ...
        for (std::size_t i=0; i< (1u<<conformational_changes.size()); ++i)
        {
            ...
            {
                state_to_conformer[c]=conformer_.size();
                std::size_t n=1;
                auto permute=rotate(c,p*n);
                while(c!=permute)
                {
                    state_to_conformer[permute]=conformer_.size();
                    ++n;
                    permute=rotate(c,p*n);
                }
                conformer_.push_back(c);
        ...
        for (std::size_t i=0; i<conformer_.size(); ++i)
        {
            ...
            for (std::size_t k=0; k<c.size(); ++k)
            {
                ...
                if (c[k]=="")
                    ...
                    change[k]="";
                auto j=state_to_conformer[change];
            ...
            transitions_.push_back(std::move(myTransition));
        }
...
private:
    ...
    std::vector<std::string> rotate(const std::vector<std::string>& x, std::size_t n)
    {
    ...
    std::size_t periodicity(const std::vector<std::string> & x)
    {
        std::size_t p=1;
        auto t=rotate(x,p);
        while(t!=x && p<x.size())
