...
public:
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(
            grammar::field(C<self_type>{}, "Parameters_distribution",
...
public:
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(
            ...
            grammar::field(C<self_type>{}, "likelihood",
                           &self_type::likelihood),
    ...
    static auto get_data_index_static() {
        ...
        return Concatenate_tuple_static(
            std::make_tuple(make_data_static(
                std::tuple<>(),
                std::make_tuple(F_s(s_beta{}, &self_type::beta)))),
            Compose_static(
                F_s(s_thermo{},
            ...
            Compose_static(
                F_s(
                    s_prior{}, [](const self_type &s) -> auto const & {
                        return s.prior();
                    }),
                base_type::get_data_index_static()),
            Compose_static(F_s(
                               s_lik{}, [](const self_type &s) -> auto const & {
                                   return s.likelihood();
                               }),
                           base_type::get_data_index_static()));
    }
...
public:
    ...
    ThPartialDlogLikelihood(const DlogLikelihood &prior,
                            const PartialDLogLikelihood<Aux...> &lik,
                            double beta)
        : DlogLikelihood(prior.logL() + beta * lik.logL(),
                         prior.elogL() + beta * lik.elogL(),
                         prior.vlogL() + beta * lik.vlogL(),
...
public:
    ...
    auto compute_DLikelihood(const Parameters &x, const mySample &current,
        ...
        if (!l.has_value())
            ...
            return Op(ThDlogLikelihood(std::move(p).value(),
    ...
    auto compute_Likelihood(const Parameters &x) const {
        ...
        if (!l.has_value())
            ...
            return Op(ThlogLikelihood(std::move(p).value(),
                                      std::move(l).value(), beta()));
    }
...
private:
    ...
    getModels(const PriorModel &prior, const LikelihoodModel &lik,
        ...
        for (std::size_t i = 0; i < beta.size(); ++i)
            out.push_back(Thermodynamic_Model<PriorModel, LikelihoodModel>(
                prior, lik, beta[i]));
        return out;
    }
};
    ...
    public:
...

        {
            ...
            return Concatenate_tuple_static(
                std::make_tuple(make_data_static(
                    std::tuple<>(),
                    std::make_tuple(F_s(CT_s<s_propDist, s_LM_landa>{},
                                        &self_type::landa)))),
                Compose_static(
                    F_s(s_propDist{},
                        [](const self_type &s) -> base_type const & {
                            return s;
                        }),
    ...
    struct myAcceptProb {
        ...
        struct myParameters {
            ...
            constexpr static auto className =
                my_static_string("Accept_parameters");
            ...
            static auto get_data_index_static() {
                return std::make_tuple(make_data_static(
                    std::tuple<>(),
                    std::make_tuple(
                        F_s(s_LM_landa{}, &self_type::landa_50),
                        F_s(s_hill_coeff{}, &self_type::hill_coeff))));
            }

            static auto get_constructor_fields() {
                return std::make_tuple(
                    grammar::field(C<self_type>{}, "landa_50",
                                   &self_type::landa_50),
    ...
    calculate_Distribution(const Model &, const Parameters x,
        ...
        if (cov) {
            ...
            if (chol_cov.has_value()) {
                return Op(Distribution(
                    typename Distribution::base_type(newpoint, cov.value(),
                                                     inv_cov, chol_cov.value()),
                    landa_));
            } else
                return Op(false,
            ...
            return Op(false,
                      "cannot invert Hessian landa matrix " + cov.error());
    }
...
public:
    ...
    constexpr static auto const className = my_static_string("mcmc_sample") +
        my_trait<base_type>::className + my_trait<template_types>::className;
    ...
    static std::vector<std::string> data_index_titles() {
        ...
        out = concatenate_unique(std::move(out),
                                 Distribution::data_index_titles());
        return out;
    }
        ...
        return Concatenate_tuple_static(
            std::make_tuple(
                ...
                make_data_static(
                    std::make_tuple(I_s(
                        i_Parameter{},
                        [](const self_type &self) { return self.x().size(); },
                        0)),
                    std::make_tuple(
            ...
            Compose_static(
                [](const self_type &self) -> auto const & {
...
calculate(const Model &M, const Distribution_Generator &G, Parameters &&x,
    ...
    if (!logL.has_value())
        return Op(false,
                  "cannot compute logLikelihood because " + logL.error());
...
public:
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(
            grammar::field(C<self_type>{}, "max_number_of_trials",
    ...
    static double Acceptance(const mcmcsample &current,
        ...
        if constexpr (Hastings) {
            ...
            double logA = (logL_candidate + logP_backward) -
                (logL_current + logP_forward);
    ...
    start(const Model &M, std::mt19937_64 &mt, const Distribution_Generator &G,
        ...
        return Op(false,
                  "fails to start after n=" + ToString(n) +
                      " trials. Error: " + error);
    }
...
public:
    ...
    myOptional_t<void> next(const Model &M, std::mt19937_64 &mt,
        ...
        auto G = adaptive.sample(mt);
        auto gg = G.calculate_Distribution(M, current.x(),
                                           current.likelihood_result());
        if (!gg) {
            return myOptional_t<void>(
                false, "cannot calculate distribution " + gg.error());
        } else {
            //    std::cerr<<"\nold current.g()\n"<<current.g();
...
public:
    ...
    static auto get_data_index_static() {
        return Compose_static(
            std::make_tuple(I_s(
                i_walker{},
                [](const self_type &self) { return self.walkers().size(); },
                0)),
    ...
    getWalker(const Model &model, Random_Generator &mt,
        ...
        while (true) {
            ...
            if (logL) {
                ...
                return Op(mcmc_s(std::move(x), std::move(logL).value(),
                                 std::move(d)));
            } else if (n < n_tries) {
                ++n;
                error += logL.error();
...
public:
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(
            grammar::field(C<self_type>{}, "numWalkers",
                           &self_type::numWalkers),
    ...
    public:
        ...
        static myOptional_t<LikelihoodResult>
        compute_Likelihood(const Model &m, Parameters &x) {
            return m.compute_Likelihood(x);
        }
        ...
        static void move(const Model &model, std::mt19937_64 &mt,
            ...
            if (!lik) {
                ...
                auto logA =
                    Acceptance(z, lik.value(), current, candidate.size());
    ...
    public:
        ...
        static auto get_constructor_fields() {
            return std::make_tuple(
                ...
                grammar::field(C<self_type>{}, "alfa_map",
                               &self_type::alfa_map),
        ...
        static Data_Index_scheme data_index() {
            ...
            out = concatenate(
                ...
                Probability_map<double>::data_index("stretch_alfa",
                                                    "stretch_alfa"));
            ...
            return Concatenate_tuple_static(
                ...
                Compose_static(
                    std::make_tuple(I_s(
                        ...
                        0)),
                    F_s(
                        CT_s<s_stretch_alpha, s_obs, s_probability>{},
                        [](const self_type &self,
                           std::size_t i_a) -> auto const & {
    ...
    start(const Model &model, Randome_Engine &mt,
        ...
        if (w)
            return Op(
                emcee_sample<Model, Adaptive_Mover>(std::move(w).value()));
        else {
...
#pragma omp parallel for
        for (std::size_t ii = 0; ii < one.size(); ++ii) {
            ...
            Mover::move(model, current.mt(i), current.Walker(i).g(), current,
                        one, current.Walker(i));
        }
        for (std::size_t ii = 0; ii < one.size(); ++ii) {
            auto i = one[ii];
...
#pragma omp parallel for
        for (std::size_t ii = 0; ii < two.size(); ++ii) {
            ...
            stretch_move<Model>::move(model, current.mt(i),
                                      current.Walker(i).g(), current, two,
                                      current.Walker(i));
        }
        for (std::size_t ii = 0; ii < two.size(); ++ii) {
            auto i = two[ii];
    ...
    Op_void next(const Model &model, Random_Engine &mt,
                 Adaptive_Mover &adaptive, Sample &current) const {
        ...
        for (std::size_t ii = 0; ii < two.size(); ++ii) {
            auto i = two[ii];
            current.Walker(i).set_g(adaptive.sample(mt));
            stretch_move<Model>::move(model, mt, current.Walker(i).g(), current,
                                      two, current.Walker(i));
        }
        for (std::size_t ii = 0; ii < two.size(); ++ii) {
            auto i = two[ii];
            adaptive.push_outcome(current.Walker(i).g(), current.accept(i));
    ...
    auto compute_PartialDLikelihood(const Model &M, const mySample &current,
                                    std::ostream &os) const {
        std::vector<std::decay_t<decltype(M.compute_PartialDLikelihood(
                                               current.Walker(0).x(),
                                               current.Walker(0), os)
                                              .value())>>
...
public:
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(
            ...
            grammar::field(C<self_type>{}, "gainMoment",
                           &self_type::getGainMoment),
    ...
    void push_rejection(const Parameterized_Distribution &landa) {
        parDist_ =
            logBayes_rule(Log_of(Complement_prob(lik_)), landa, parDist_);
    }
  ...
  */
    ...
    Adaptive_Parameterized_Distribution_Generator(
        ...
        : lik_{}, g_{}, gainMoment_(gainMoment), p_{landa},
          parDist_{Likelihood::uniform_parameter_prior(par, 0.0), 0},
          logEvidence{0} {}
    ...
    Adaptive_Parameterized_Distribution_Generator(
        ...
        double logEvide)
        : lik_{lik}, g_{g}, gainMoment_{gainMoment}, p_{pmap},
          parDist_{pardist}, logEvidence{logEvide} {}
        ...
        return Concatenate_tuple_static(
            std::make_tuple(
                make_data_static(std::tuple<>(),
                                 std::make_tuple(F_s(s_logEvidence{},
                                                     &self_type::getLogEvidence)

                                                     )),
                make_data_static(
                    std::make_tuple(I_s(
                        typename Parameterized_Distribution::index_type{},
                        [](const self_type &self) {
                    ...
                    std::make_tuple(
                        ...
                        F_s(CT_s<
                                typename Parameterized_Distribution::label_type,
                                s_use, s_probability>{},
                            [](const self_type &self, std::size_t i_par) {
            ...
            Compose_static(
                F_s(s_obs{}, &self_type::get_Parameters_Map),
...
public:
    ...
    void push_rejection(const DistGen &landa) {
    ...
    static auto get_data_index_static() {
        return Concatenate_tuple_static(
            std::make_tuple(make_data_static(
                std::make_tuple(I_s(
                    typename DistGen::index_type{},
                    [](const self_type &self) {
                        return self.get_landa_Map().x().size();
                    },
                    0)),
                std::make_tuple(F_s(typename DistGen::label_type{},
                                    [](const self_type &s, std::size_t i_a) {
                                        return s.get_ProbabilityMap().x()[i_a];
                                    }),
                                F_s(CT_s<typename DistGen::label_type, s_use,
                                         s_probability>{},
                                    [](const self_type &s, std::size_t i_a) {
            ...
            Compose_static(
                std::make_tuple(I_s(
                    ...
                    0)),
                F_s(
                    CT_s<typename DistGen::label_type, s_obs, s_probability>{},
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(grammar::field(C<self_type>{}, "probabilityMap",
                                              &self_type::get_ProbabilityMap),
                               grammar::field(C<self_type>{}, "historyMap",
                                              &self_type::get_landa_Map),
                               grammar::field(C<self_type>{},
                                              "target_probability",
...
public:
    ...
    public:
        ...
        typedef Thermodynamic_Model_Series<priorModel, LikelihoodModel>
            ModelSeries;
        ...
        template <class Random_Engine>
        static myOptional<samples, reg_tag> evaluate(
...
#pragma omp parallel for
        ...
        static auto get_constructor_fields() {
            return std::make_tuple(grammar::field(C<self_type>{}, "i_to_scout",
                                                  &self_type::get_i_to_scout),
                                   grammar::field(C<self_type>{}, "scouts",
                                                  &self_type::get_scouts));
        }
        std::map<std::size_t, std::size_t> const &get_i_to_scout() const {
            return i_to_scout;
        }
        std::vector<
            mcmc_sample_t<Model, Adapative_Distribution_Generator>> const &
        get_scouts() const {
            return scouts_;
        }
        ...
        static Data_Index_scheme data_index() {
            ...
            out = concatenate(
                std::move(out),
                mcmc_sample_t<Model,
                              Adapative_Distribution_Generator>::data_index());
            return out;
        }
        ...
        static std::vector<std::string> data_titles() {
            ...
            auto v =
                mcmc_sample_t<Model,
                              Adapative_Distribution_Generator>::data_title();
    ...
    static double Evidence(const Model &m, const samples &s) {
        ...
        for (std::size_t i = 1; i < m.size(); ++i) {
            ...
            if ((i == n - 1) && (beta > 0)) {
                double db0 = beta;
                sum += (db0 * (1 + db0 / db / 2)) * logLik -
                    sqr(db0) / db / 2 * logLik0;
                sumdb += db0;
            }
            beta0 = beta;
            logLik0 = logLik;
    ...
    start(const Thermodynamic_Model_Series<priorModel, LikelihoodModel> &model,
        ...
        return samples<
            priorModel, LikelihoodModel,
            Adapative_Distribution_Generator>::evaluate(get_Metropolis(), model,
                                                        mt, g);
    }
...
#pragma omp parallel for
    ...
    auto compute_PartialDLikelihood(
        ...
        std::vector<std::decay_t<
            decltype(model.model(0)
                         .compute_PartialDLikelihood(
                             current.Scout(0).x(), current.Scout(0),
...
#pragma omp parallel for
    ...
    Adaptive_Metropolis_H<Hastings> const &get_Metropolis() const {
        return amh_;
    }
...
public:
    ...
    static auto get_constructor_fields() {
        return std::make_tuple(
            grammar::field(C<self_type>{}, "ij_history",
                           &self_type::ij_history),
        ...
        return Concatenate_tuple_static(
            std::make_tuple(make_data_static(
                std::make_tuple(
                    I_s(
                        i_beta{},
                        [](const self_type &self) { return self.size(); }, 0),
                    I_s(
                        i_walker{},
            ...
            Compose_static(
                std::make_tuple(I_s(
                    i_beta{}, [](const self_type &self) { return self.size(); },
                    0)),
                [](const self_type &self, std::size_t i_b) -> auto const & {
                    return self.Scout(i_b);
                },
                emcee_sample<subModel,
                             Adaptive_Mover>::get_data_index_static()));
    }
    ...
    void swap(std::size_t i, std::size_t j, std::size_t k, const Model &model) {
        ...
        //              << "log_k="
        //              << Scout(i +
        //              1).Walker(k).likelihood_result().likelihood().logL()
        //              << "\n";
    ...
    evaluate(const Ensemble_Metropolis_Hastings &emh, const Model &m,
        ...
        if (res)
            return Op(
                Ensemble_samples(std::move(ij_history), std::move(scouts_)));
        else {
            return Op(false, res.error());
        }
    }
...
private:
    ...
    init_history(std::size_t nScouts, std::size_t nWalkers) {
        std::vector<std::vector<std::pair<std::size_t, std::size_t>>> out(
            nScouts,
            std::vector<std::pair<std::size_t, std::size_t>>(nWalkers));
...
#pragma omp parallel for
        for (std::size_t i = 0; i < model.size(); ++i) {
            op[i] = get_Ensamble_Metropolis().next(
                model.model(i), mt[i], adaptives[i], current.Scout(i));
        }
...
#pragma omp parallel for
    ...
    auto compute_PartialDLikelihood(const Model &M, const mySample &current,
                                    std::ostream &os) const {
        std::vector<
            decltype(get_Ensamble_Metropolis().compute_PartialDLikelihood(
...
struct OutputGenerator {
    typedef Cs<Metropolis_algorithm, ModelSeries, Distribution_Generator>
        template_types;
    constexpr static auto const className =
        my_static_string("OutputGenerator_") +
        my_trait<template_types>::className;
    ...
    void load(const std::string &idname, std::string &info,
              Metropolis_algorithm &mcmc, ModelSeries &M,
              std::size_t &nsamples) {
    ...
    void save(const std::string &idname, std::size_t isample,
              const State &state, const Distribution_Generator &G) {
    ...
    void print_element(const State_Data_Index_tuple &statedata,
        ...
        std::tuple<std::size_t, State const *, ModelSeries const *> s(
            isample, &state, &M);
    ...
    for (; isample < nsamples; ++isample) {
        ...
        if (!res)
            return Op_void(false,
                           " interrupted at sample i=" + ToString(isample) +
                               res.error());

        if (output.max_index_ana(isample) > 0) {
            auto ana_sample =
                mcmc.compute_PartialDLikelihood(M, state, std::cerr);

            output.print_element(statedata, gendata, anadata, idname, isample,
                                 M, state, G, ana_sample);

        } else {
            output.print_element(statedata, gendata, idname, isample, M, state,
                                 G);
        }
...
Op_void run_Montecarlo_Markov_Chain(const std::string idname,
    ...
    typedef std::decay_t<decltype(mcmc.compute_PartialDLikelihood(
        M, state.value(), std::cerr))>
        AnaState;
...
Op_void continue_Montecarlo_Markov_Chain(
    ...
    return continue_Montecarlo_Markov_Chain(
        idname, isample, state, mts, mcmc, M, G, nsamples, data_output, output);
}
...
Op_void run_Thermo_Levenberg_ProbVel(
    ...
    //     Adaptive;
    //  typedef  Thermodynamic_Model_Series<PriorModel, Likelihood_Model>
    //  Models; typedef Parallel_Tempering<true> MCMC;
    ...
    std::vector<Adaptive_Parameterized_Distribution_Generator<
        LevenbergMarquardt<Model>>>
        ala(betas.size(),
            Adaptive_Parameterized_Distribution_Generator<
