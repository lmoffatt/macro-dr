...
*/
    ...
    SingleLigandModel SM(A.Qs(P),A.g(P),-60.0,1000,0,1e-9);
    ...
    Markov_Model_calculations<Markov_Transition_step,SingleLigandModel,experiment::basic_Experiment<experiment::point<double,double>,measure_just_y<double>>,double>
            MC(SM,e,10,1e-7);


    auto s=simulate<decltype (e),Allosteric_Model>::run(0,e,A,P,10,1e-9, 1e-7);
    auto ds=experiment::Experiment_to_DataFrame(s.value());
    ds.write(std::cout);
    write(std::cout,s.value());
