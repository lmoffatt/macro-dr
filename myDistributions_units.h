#ifndef MYDISTRIBUTIONS_UNITS_H
#define MYDISTRIBUTIONS_UNITS_H
...
template <class unit, class unit2=decltype (unit{}*unit{})>
auto gaussian_function(v<double,unit> x, v<double,unit> mean, v<double,unit2> variance){
    return exp(-sqr(x-mean)/variance/2.0)/sqrt((2*PI)*variance);}

template <class unit>
auto gaussian_function(v<double,unit> x, v<double,unit> mean, v<double,unit> stddev){
    return exp(-sqr((x-mean)/stddev)/2.0)/(sqrt(2*PI)*stddev);}


template <class unit, class unit2=decltype (unit{}*unit{})>
auto log_gaussian_function(v<double,unit> x, v<double,unit> mean, v<double,unit2> variance)
{ return -sqr(x-mean)/variance/2.0-std::log(2*PI)/2.0-log(sqrt(variance));}
template <class unit>
auto log_gaussian_function(v<double,unit> x, v<double,unit> mean, v<double,unit> stddev)
{ return -sqr(x-mean)/variance/2.0-std::log(2*PI)/2.0-log(stddev);}


//inline double expected_log_gaussian(double varianc) { return -0.5 * (1.0 + log(2 * PI * variance));}


template <typename T,class unit> class Normal_Distribution;
...
template <typename T, class unit> class Base_Distribution {
public:
    virtual Base_Distribution *clone() const = 0;
    virtual std::ostream &put(std::ostream &os) const = 0;
    typedef mult_exponent_t<unit,-1> unit_1;
    typedef mult_exponent_t<unit,2> unit2;
    typedef mult_exponent_t<unit,-2> unit_2;
    typedef mult_exponent_t<unit,-4> unit_4;

    constexpr static auto const className =
        my_static_string("Base_Distribution_") + my_trait<T>::className;
    virtual std::string myClass() const = 0;

    virtual v<T,unit> sample(std::mt19937_64 &mt) const = 0;

    virtual v<double, unit_1 > p(const T &x) const = 0;

    virtual logv<double, unit_1 > logP(const T &x) const = 0;

    virtual logv<double, unit_2 > vlogP(const T &x) const = 0;

    virtual v<M_Matrix<T>,unit> param() const = 0;

    // virtual M_Matrix<T> Score(const T& x)const =0;    // V(x)

    virtual v<M_Matrix<T>,unit_2> Fisher_Information() const = 0; // I()

    virtual v<T,unit_1> dlogL_dx(const T &x) const = 0;

    virtual v<T,unit_2> dlogL_dx2(const T &x) const = 0;

    virtual v<T,unit> mean() const = 0;

    virtual v<T,unit> stddev() const = 0;
...
template <class unit> struct Derived_types<Base_Distribution<double,unit>> {
    typedef Cs<Normal_Distribution<double,unit>, Beta_Distribution<unit>,
               stretch_move_Distribution<unit>>
        type;
    constexpr bool static value = true;
};

/*inline double logit(v<double,dimension_less> x) { return std::log(x / (1.0 - x)); }
...
template <class unit>
class Normal_Distribution<double,unit> : public Base_Distribution<double,unit> {
public:
    typedef Base_Distribution<double,unit> base_type;
    ...
    typedef Normal_Distribution<double,unit> self_type;
    ...
    virtual Normal_Distribution<double,unit> *clone() const override {
        return new Normal_Distribution<double,unit>(*this);
    };

    virtual v<double,unit> sample(std::mt19937_64 &mt) const override {
        return v<double,unit>(std::normal_distribution<double>{param_[0], param_[1]}(mt));
    }

    virtual v<double,unit_1> p(const v<double,unit> &x) const override {
        return 1.0 / (std::sqrt(2 * PI) * stddev()) *
            std::exp(-0.5 * sqr((x - mean()) / stddev()));
    }

    virtual logv<double,unit_1> logP(const v<double,unit> &x) const override {
        return -0.5 * std::log(2 * PI) - log(stddev()) -
            0.5 * sqr((x - mean()) / stddev());
    }
    virtual logv<double,unit_2> vlogP(const v<double,unit> &x) const override {
...
*/
    virtual v<M_Matrix<double>,unit_2> Fisher_Information() const override {
        return M_Matrix<double>(
            2, 2, Matrix_TYPE::DIAGONAL,
            std::vector<double>{d2logL_dmean2().value(), d2logL_dvariance2().value()});
    };

    virtual v<double,unit> mean() const override { return param_[0]; }

    virtual v<double,unit> stddev() const override { return std::sqrt(param_[1]); };

    virtual v<double,unit2> variance() const { return param_[1]; }

    virtual M_Matrix<double> param() const override { return param_; }

    Normal_Distribution<double>(v<double,unit> mean, v<double,unit2> variance)
        : param_(1, 2, std::vector<double>{mean, variance}) {}

    Normal_Distribution() = default;
    virtual ~Normal_Distribution() {}
    virtual v<double,unit_1> dlogL_dx(const v<double,unit> &x) const override {
        return (mean() - x) / variance();
    }

    virtual v<double,unit_2> dlogL_dx2(const double &) const override {
        return -1.0 / variance();
    }

    virtual logv<double,unit_1> expected_logP() const override {
        return -0.5 * (1.0 + log(2 * PI))-log(stddev());
    }

    virtual v<double,dimension_less> variance_logP() const override { return 0.5; }
...
protected:
    ...
    v<double,unit_2> d2logL_dmean2() const { return 1.0 / variance(); }
    v<double,unit_2> d2logL_dstddev2() const { return 0.5 / variance(); }

    v<double,unit_4> d2logL_dvariance2() const { return 0.5 / sqr(variance()); }
};

template <typename E, class unit>
class Normal_Distribution<M_Matrix<E>,unit> : public Base_Distribution<M_Matrix<E>,unit> {
public:
    constexpr static auto const className =
    ...
    typedef Normal_Distribution<M_Matrix<E>,unit> self_type;
    static auto get_constructor_fields() {
        return std::make_tuple(
            grammar::field(C<self_type>{}, "mean", &self_type::mean),
    ...
    v<M_Matrix<E>,unit> sample(std::mt19937_64 &mt) const override {
        M_Matrix<E> r;
    ...
    virtual v<M_Matrix<M_Matrix<E>>,unit_2> Fisher_Information() const override {
        return M_Matrix<M_Matrix<E>>(
            2, 2, Matrix_TYPE::DIAGONAL,
            std::vector<M_Matrix<E>>{d2logL_dmean2(), d2logL_dCov2()});
    }

    logv<double,unit_2> logP(const v<M_Matrix<E>,unit> &x) const override {
        if (param_.size() > 0)
            return -0.5 * (mean().size() * log(PI) + logDetCov() + chi2(x));
        else
            return std::numeric_limits<double>::quiet_NaN();
    }
    v<double,dimension_less> vlogP(const v<M_Matrix<E>,unit> &x) const override {
        return sqr(logP(x) - expected_logP());
    }

    v<double,unit> p(const v<M_Matrix<E>,unit> &x) const override { return std::exp(logP(x)); }
...
template<class unit>
class Beta_Distribution : public Base_Distribution<double,unit> {
public:
    typedef Base_Distribution<double,unit> base_type;
    ...
    virtual Base_Distribution<double,unit> *clone() const override {
        return new Beta_Distribution(*this);
    }

    Beta_Distribution(v<double,unit> alfa, v<double,unit> beta)
    ...
    v<double,unit> p() const { return mean(); }

    void push_accept() { ++a_[0]; }
    void push_reject() { ++a_[1]; }

    v<double,dimension_less> sample(std::mt19937_64 &mt) const override {
...
template<>
class stretch_move_Distribution<dimension_less> : public Base_Distribution<double,dimension_less> {
...
public:
    ...
    virtual v<double,dimension_less> sample(std::mt19937_64 &mt) const override {
        std::uniform_real_distribution<double> U(std::sqrt(1.0 / a_[0]),
                                                 std::sqrt(a_[0]));
        double z = sqr(U(mt));
        return z;
    }
    virtual v<double,dimension_less> p(const v<double,dimension_less> &x) const override {
        return std::pow(x.value(), -0.5) / Z_;
    }
    virtual v<double,dimension_less> logP(const v<double,dimension_less> &x) const override {
        return -0.5 * std::log(x.value()) - log(Z_);
    }
    virtual v<double,dimension_less> vlogP(const double &x) const override {
        return sqr(logP(x) - expected_logP());
    }
